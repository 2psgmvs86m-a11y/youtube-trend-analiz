...
translations = {
    'tr': {
        ...
        'warn_monetization': 'Bu kanalın para kazanma özelliği kapalı görünüyor.',
    ...
    'en': { 'title': 'YouTube Audit', 'active': 'ACTIVE ✅', 'passive': 'INACTIVE ❌', 'error': 'Error' },
    'de': { 'title': 'YouTube Audit', 'active': 'AKTIV ✅', 'passive': 'INAKTIV ❌', 'error': 'Fehler' }
}
...
def calculate_grade(sub_count, view_count, video_count):
    ...
    if sub_count > 500000 and engagement_rate > 2: return "A"
    if engagement_rate > 10: return "A"
    if engagement_rate > 5: return "B"
    return "C"

def get_niche_cpm(tags, title, desc):
    return 2.00, "Genel"

# --- KESİN SONUÇ VEREN MONETIZATION KONTROLÜ ---
def check_real_monetization(channel_id):
    """
    Sadece ve sadece Proxy ile bağlanır.
    Kaynak kodunda kesin kanıt arar.
    Asla tahmin yürütmez.
    """
    ...
    try:
        # Timeout süresini biraz artırdık (10sn) çünkü proxyler yavaş olabilir
        response = requests.get(url, headers=headers, proxies=proxies, timeout=10)
        text = response.text
        
        # 1. KESİN KANIT: Monetization Enabled etiketi
        if '"key":"is_monetization_enabled","value":"true"' in text:
            return True
            
        # 2. KESİN KANIT: Katıl Butonu
        if 'sponsorButtonRenderer' in text:
            return True
            
        # 3. KESİN KANIT: Ürün Rafı
        if 'merchandiseShelfRenderer' in text:
            return True
            
        # Eğer hiçbiri yoksa kesinlikle KAPALIDIR.
        return False

    except Exception as e:
        print(f"Proxy Hatası ({proxy_url}): {e}")
        # Proxy hatası alınırsa, bir kez daha denemek yerine False dönüyoruz.
        # (Kullanıcı F5 atarak tekrar dener)
        return False
# -----------------------------------------------
...
def get_channel_data(query, lang_code='tr'):
    ...
    # --- KRİTİK DEĞİŞİKLİK ---
    # Artık "Kanal büyükse aç" mantığı yok.
    # Sadece Proxy kontrolü ne derse o.
    is_monetized = False
    
    # Sadece 1000 abone üstü için zahmete gir (API limiti yememek için değil, Proxy yormamak için)
    if sub_count >= 1000:
    ...
    if request.method == 'POST':
        ...
        if query:
            try:
                ...
                print(f"Hata: {e}")
                error = "API Hatası veya Proxy Bağlantı Sorunu"
