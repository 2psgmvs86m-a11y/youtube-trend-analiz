...
# --- GELİŞTİRİLMİŞ MONETIZATION KONTROLÜ ---
def check_real_monetization(channel_id):
    # URL sonuna ?hl=en ekleyerek İngilizce içerik zorluyoruz (Parsing daha kolay)
    ...
    try:
        ...
        # 1. Klasik Yöntem (Doğrudan Etiket)
        if '"key":"is_monetization_enabled","value":"true"' in text: return True
        
        # 2. Katıl Butonu (Sponsor Button) - Kesin Kanıt
        if 'sponsorButtonRenderer' in text: return True
        
        # 3. Ürün Rafı (Merch Shelf) - Kesin Kanıt
        if 'merchandiseShelfRenderer' in text: return True
        
        # 4. Alternatif Yazım (Bazen JSON formatı değişir)
        if '"isMonetizationEnabled":true' in text: return True
        
        # 5. YENİ: Script içi kontrol (ytInitialData içinde derin arama)
        if 'badgetype_channel_member' in text.lower(): return True # Üye rozeti varsa açıktır
...
def get_channel_data(query, lang_code='tr'):
    ...
    is_monetized = False
    # Sadece abone sayısı 1000 üzerindeyse sayfa kaynağını kontrol et
    if sub_count >= 1000:
    ...
    if request.method == 'POST':
        ...
        if query:
            try:
                ...
                print(f"Hata: {e}")
                error = "API Hatası veya Proxy"

    return render_template('index.html', content=content, current_lang=lang, result=result, error=error)

# --- TEST SAYFASI (PROXIES KONTROLÜ) ---
@app.route('/test-proxy')
def test_proxy_page():
    results = []
    results.append("<h1>Proxy Test Raporu</h1><ul>")
    ...
    for i, proxy in enumerate(PROXIES):
        ...
        try:
            start = datetime.now()
            # Google'a istek at (YouTube bazen ağır olabilir, Google daha hızlıdır test için)
